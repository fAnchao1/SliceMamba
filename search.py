...
from tester import get_cand_miou_dice

#tester_synapse   tester_polyp
...
class EvolutionSearcher(object):
    ...
    def __init__(self, args,config):
        ...
        self.model = SMUNet(
            ...
            depths_decoder=self.model_cfg['depths_decoder'],
            drop_path_rate=self.model_cfg['drop_path_rate'],
            load_ckpt_path=self.model_cfg['load_ckpt_path'],
        )
        self.model.cuda()
        supernet_state_dict = torch.load(self.model_cfg['supernet_path'])
    ...
    def is_legal(self, cand):
        ...
        info['miou_with_dice'] = get_cand_miou_dice(self.model, cand)
    ...
    def update_top_k(self, candidates, *, k, key, reverse=True):
        '''
        miou,dice reverse=True. bigger means high performance
        '''
    ...
    def search(self):
        ...
        while self.epoch < self.max_epochs:
            ...
            self.update_top_k(
                self.candidates, k=self.select_num, key=lambda x: self.vis_dict[x]['miou_with_dice'])
            self.update_top_k(
                self.candidates, k=self.population_num, key=lambda x: self.vis_dict[x]['miou_with_dice'])

            print('epoch = {} : top {} result'.format(
                self.epoch, len(self.keep_top_k[self.population_num])))
            for i, cand in enumerate(self.keep_top_k[self.population_num]):
                print('No.{} {} Top-1 miou_with_dice = {}'.format(
                    i + 1, cand, self.vis_dict[cand]['miou_with_dice']))
                ops = [i for i in cand]
                print(ops)
...
def main():
    ...
    parser.add_argument('--log-dir', type=str, default='log_nas')
    ...
    searcher = EvolutionSearcher(args,config)
