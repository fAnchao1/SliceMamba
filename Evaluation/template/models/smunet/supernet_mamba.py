...
class BSS(nn.Module):
    ...
    def slice_h_flatten(self,slice_res):
        B,C,H,W = slice_res[0].shape
        flatten_tensors = [torch.transpose(slice,dim0=2,dim1=3).contiguous().view(B, C, -1) for slice in slice_res]
        flatten_res = torch.cat([t for t in flatten_tensors], dim=2)
        flip_res = torch.flip(flatten_res, dims=[-1])
        return torch.stack([flatten_res,flip_res],dim=1)

    def slice_w_flatten(self,slice_res):
        B,C,H,W = slice_res[0].shape
        
        flatten_tensors = [slice.contiguous().view(B, C, -1) for slice in slice_res]
        flatten_res = torch.cat([t for t in flatten_tensors], dim=2)
        flip_res = torch.flip(flatten_res, dims=[-1])
        return torch.stack([flatten_res,flip_res],dim=1)
    ...
    def forward_corev0(self, x: torch.Tensor):   ##for mamba used
        self.selective_scan = selective_scan_fn
        ...
        xs = torch.cat([x_h_flatten_res, x_w_flatten_res], dim=1) # (b, k, d, l)
...
class VSSLayer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert depth==len(self.choose_list)
        for i in range(depth):
            index = self.choose_list[i]
            if index==0:
                split_method = [2,2]
                self.blocks.append(S3Block(
                                        hidden_dim=dim,
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, architecture=None,depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
        ...
        assert architecture is not None and len(architecture)==22
        self.archi_encoder = architecture[:15]
        self.archi_decoder = architecture[15:]
        ...
        for i_layer in range(self.num_layers):
            if i_layer==0:
                choose_list = self.archi_encoder[:2]
            elif i_layer==1:
                choose_list = self.archi_encoder[2:4]
            elif i_layer==2:
                choose_list = self.archi_encoder[4:13]
            else:
                choose_list = self.archi_encoder[13:]
            layer = VSSLayer(
                dim=dims[i_layer],
        ...
        for i_layer in range(self.num_layers):
            if i_layer==0:
                choose_list = self.archi_decoder[:2]
            elif i_layer==1:
                choose_list = self.archi_decoder[2:4]
            elif i_layer==2:
                choose_list = self.archi_decoder[4:6]
            else:
                choose_list = self.archi_decoder[6:]
            layer = VSSLayer_up(
