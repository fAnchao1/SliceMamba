...
def cal_params_flops(model, size, logger):
    ...
    flops, params = profile(model, inputs=(input,architecture()))
...
def save_mask(msk_pred, i,save_path ,threshold=0.5, test_data_name=None):

    msk_pred = np.where(np.squeeze(msk_pred, axis=0) > threshold, 1, 0)
    img_path = save_path +str(i) +'.png'
    cv2.imwrite(img_path,msk_pred*255.) 