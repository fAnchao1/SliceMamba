...
class Skin_datasets(Dataset):
    def __init__(self, path_Data, config, type='train'):
        super(Skin_datasets, self)
        if type=='train':
            ...
            images_list = sorted(os.listdir(path_Data+'search/images/'))
            masks_list = sorted(os.listdir(path_Data+'search/masks/'))
            self.data = []
            for i in range(len(images_list)):
...
class Poly_datasets(Dataset):
    def __init__(self, path_Data, config, type='train', test_dataset = None):
        super(Poly_datasets, self)
        if type=='train':
            ...
            images_list = sorted(os.listdir(path_Data+'searchDataset/images/'))
            masks_list = sorted(os.listdir(path_Data+'searchDataset/masks/'))
            self.data = []
            for i in range(len(images_list)):
                img_path = path_Data+'searchDataset/images/' + images_list[i]
                mask_path = path_Data+'searchDataset/masks/' + masks_list[i]
                self.data.append([img_path, mask_path])
            self.transformer = config.train_transformer
...
class Synapse_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.split == "train" or self.split=='search':
